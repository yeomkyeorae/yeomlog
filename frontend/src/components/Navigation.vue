  ...
  <nav class="flex items-center justify-between flex-wrap bg-pink-300 p-6">
    ...
    <div :class="open ? 'block': 'hidden'" class="w-full flex-grow sm:flex sm:items-center sm:w-auto">
      <div class="text-sm sm:flex-grow">
        <button :to="cv.path" class="no-underline block mt-4 sm:inline-block sm:mt-0 text-white hover:text-white mr-4">
          <i class="material-icons">{{cv.icon}}</i>Curriculum Vitae
        </button>
        <button :to="projects.path" class="no-underline block mt-4 sm:inline-block sm:mt-0 text-white hover:text-white mr-4">
